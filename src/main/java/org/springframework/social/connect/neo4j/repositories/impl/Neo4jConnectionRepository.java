...
public class Neo4jConnectionRepository implements ConnectionRepository{
    ...
    public MultiValueMap<String, Connection<?>> findAllConnections() {
        ...
        Sort sort = new Sort(Sort.Direction.ASC, "providerId").and(new Sort(Sort.Direction.ASC, "rank"));
        List<SocialUserConnection> dbConnections = repository.findByUserId(userId, sort, 0);
    ...
    public List<Connection<?>> findConnections(String providerId) {
        ...
        List<SocialUserConnection> dbConnections = repository.findByUserIdAndProviderId(userId, providerId, sort, 0);
    ...
    public MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUsers) {
        ...
        for (Iterator<Map.Entry<String, List<String>>> it = providerUsers.entrySet().iterator(); it.hasNext();) {
            ...
            Sort sort = new Sort(Sort.Direction.ASC,"providerId").and(new Sort(Sort.Direction.ASC,"rank"));
            dbConnections.addAll(repository.findByUserIdAndProviderIdAndProviderUserIdIn(userId, providerId, entry.getValue(), sort, 0));
        }
    ...
    public void addConnection(Connection<?> connection) {
        ...
        Integer rank = 0;
        Sort sortRank = new Sort(Sort.Direction.DESC,"rank");
        List<SocialUserConnection> existingCons = repository.findByUserIdAndProviderId(userId, data.getProviderId(), sortRank, 0);
        if(existingCons.size()>0) {
            rank = existingCons.get(0).rank;
        }
    ...
    private Connection<?> findPrimaryConnection(String providerId) {
        ...
        Sort sort = new Sort(Sort.Direction.ASC,"rank");
        List<Connection<?>> connections = getConnectionsListFromDbConnections(repository.findByUserIdAndProviderId(userId,providerId,sort,0));
    ...
    private List<Connection<?>> getConnectionsListFromDbConnections(List<SocialUserConnection> dbConnections) {
